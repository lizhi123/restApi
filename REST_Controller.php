...
class WIN_Controller extends CI_Controller
{
...
class CommonBase extends WIN_Controller
{
    public $authorInfo;
    function __construct()
    {
        ...
        $func = $this->router->fetch_method();//获取当前请求方法
        $get  = $this->input->get();
        $post = $this->input->post();
        $authorizationId    = isset($post['AuthorizationId'])   ? $post['AuthorizationId'] : NULL;
        $authorizationKey   = isset($post['AuthorizationKey'])  ? $post['AuthorizationKey'] : NULL;
        $authorizationToken = isset($post['AuthorizationToken'])? $post['AuthorizationToken'] : NULL;
        if(!$authorizationId){
            $authorizationId    = isset($get['AuthorizationId'])    ? $get['AuthorizationId'] : NULL;
        }
        if(!$authorizationKey){
            $authorizationKey   = isset($get['AuthorizationKey'])   ? $get['AuthorizationKey'] : NULL;
        }
        //authorization
        if(!$authorizationId || !$authorizationKey || (!$authorizationToken && ($func != 'authorization') && ($func != 'refreshAuth') && ($func != 'test'))){
            echo json_encode(array('ReturnCode' => 201));
            die;
        }
        $rs = $this->Login_model->getUserInfoById($authorizationId);//根据AuthorizationId 查询渠道信息
        $this->authorInfo = $rs;
        if(empty($rs)){ //渠道方认证id不存在
            echo json_encode(array('ReturnCode' => 202));
            die;
        }else{
            if($authorizationKey != $rs['authorization_key']){//验证authorizationKey
                echo json_encode(array('ReturnCode' => 203));
                die;
            }else{
                if( ($func != 'authorization') && ($func != 'refreshAuth')&& ($func != 'test') ){
                    $tokenInfo = $this->Login_model->getTokenById($rs['channelid']);
                    //如果accessToken过期或错误
                    if( empty($tokenInfo) || ($tokenInfo['access_token'] != $authorizationToken) ){
                        echo json_encode(array('ReturnCode' => 206));
                        die;
                    }
                }else{
                }
 ...
 * @version         3.0.0
 */
abstract class REST_Controller extends WIN_Controller {
...
class Oauth extends REST_Controller
{
    function __construct()
    {
        ...
        if(in_array('api',$segs) && count($segs)%2==0){
            header('HTTP/1.1 403 Forbidden');
            header("Date:".date('r',time()));
            header('Status:403 Forbidden');
            header('error_msg:URL address is not valid');
            die;
        }
        if(!in_array('api',$segs) && count($segs)%2!=0){
            header('HTTP/1.1 403 Forbidden');
        ...
        /*请求url示例:http://release-rd2.ntalker.com:2401/api/OauthCredentials/oauth?clientid=acd126&clientsecret=ww
        &request=http://release-rd2.ntalker.com:2401/api/Chatscene/Chatscene/siteid/ds
        /token/12DFDC18BDAEBADC41F7701E5CD44DD8/stime/12345/etime/454641*/
       /* $clientId=$this->get('clientid');
        $clientId=inject_check($clientId);
        $clientSecret=$this->get('clientsecret');
        $clientSecret=inject_check($clientSecret);*/
        /*$str=$this->get('request');
        $str=inject_check($str);*/
        //判断该请求用户是否有权限访问该接口
        $con = $this->router->class;
        $func = $this->router->method;
        /*echo $con;
        echo'<br>';
        echo $func;
        die;*/
        ...
        $str=$con.'-'.$func;
        //echo $str;die;
        //根据用户传递的clientid查询其拥有的访问权限"控制器-方法"
        //$clientId=$this->get('clientid');
        //$clientId=$this->security->xss_clean($clientId);
       // $clientId=inject_check($clientId);
        //验证权限
        $data=getallheader();
        $token_1=isset($data['Access-Token']) ? $data['Access-Token']:NULL;
        $token_2=isset($data['Authorization']) ? $data['Authorization']:NULL;
        $version=isset($data['X-Version']) ? $data['X-Version'] : NULL;
        if($token_1=='' && $token_2==''){
            header('HTTP/1.1 422 Unprocessable Entity');
            header("Date:".date('r',time()));
            header('Status:422 Unprocessable Entity');
            header("error_msg:Token does not exist");
            die;
        }
        if($token_1!='' && $token_2!=''){
            header('HTTP/1.1 422 Unprocessable Entity');
            header("Date:".date('r',time()));
            header('Status:422 Unprocessable Entity');
            header("error_msg:Can not pass the two token");
            die;
        }
        if($token_1!=''&& $token_2==''){
            $token=$token_1;
            $token=inject_check($token);
        ...
        //echo $token;die;
        $bool=$this->Oauth_model->auth($token);
        //echo $bool;die;
        if(!$bool){
            header('HTTP/1.1 403 Forbidden');
            header("Date:".date('r',time()));
            header('Status:403 Forbidden');
            header("error_msg:Token validation failed");
            //echo urldecode(json_encode(array('Message'=>urlencode('Token验证失败!'))));
            die;
        }
        $bools=$this->Oauth_model->expire($token);
        if(!$bools){
            header('HTTP/1.1 403 Forbidden');
            header("Date:".date('r',time()));
            header('Status:403 Forbidden');
            header("error_msg:Token invalid,please reapply or refresh Token");
            //echo urldecode(json_encode(array('Message'=>urlencode('Token失效，请重新申请或者刷新Token'))));
            die;
        }
        //echo $clientId;die;

        //限制用户能调用的接口
        $scope=$this->Oauth_model->_getscope($token);
        //print_r($scope);die;
        $scope=$scope['scope'];
        //echo $scope;die;
        /*if(!empty($token)){
        ...
        if(strpos($scope,$str)===false){
            ...
            header("Date:".date('r',time()));
            header('Status:403 Forbidden');
            header('error_msg:No permission to access');
            //header('Content-Type:application/json');
            die;
            //echo urldecode(json_encode(array('ReturnCode'=>401,'Message'=>urlencode('没有权限访问!'))));
            //die;
        }
        //限制用户调用自己的数据
        $userid=$this->Oauth_model->_getuserid($token);
        $userid=isset($userid['user_id']) ? $userid['user_id'] : NULL;

        //get请求过来的参数
        $get_data=$this->_get_args;
        $get_data=inject_check($get_data);
        $get_data=$this->security->xss_clean($get_data);
        $siteid='';
        if($get_data!=''){
            $site1=isset($get_data['siteid']) ? $get_data['siteid'] : NULL;
            $site2=isset($get_data['site_id']) ? $get_data['site_id'] : NULL;
        }
        ...
        //post请求过来的参数
        $post_data=$this->_post_args;

        $post_data=inject_check($post_data);
        $post_data=$this->security->xss_clean($post_data);
        //var_dump($sites);
        if($post_data!=''){
            $sites1=isset($post_data['siteid']) ? $post_data['siteid']: NULL;
            $sites2=isset($post_data['site_id']) ? $post_data['site_id']: NULL;
        }
        ...
        //put请求过来的参数
        $put_data=$this->_put_args;
        $put_data=inject_check($put_data);
        $put_data=$this->security->xss_clean($put_data);
        if($put_data!=''){
            $sites1=isset($put_data['siteid']) ? $put_data['siteid']: NULL;
            $sites2=isset($put_data['site_id']) ? $put_data['site_id']: NULL;
        }
        if($sites1!=''&&$sites2==''){
            $siteid=$sites1;
        }
        ...
        //delete过来的参数
        $delete_data=$this->_delete_args;
        $delete_data=inject_check($delete_data);
        $delete_data=$this->security->xss_clean($delete_data);
        if($delete_data!=''){
            $sites1=isset($delete_data['siteid']) ? $delete_data['siteid']: NULL;
            $sites2=isset($delete_data['site_id']) ? $delete_data['site_id']: NULL;
        }
        if($sites1!=''&&$sites2==''){
            $siteid=$sites1;
        }
        ...
        $_SESSION['version']=$version;
         //判断调用是否被允许
        /*echo $siteid;die;*/
        if($userid!='all'&&$siteid!=''){
            if(substr($userid,3)!=1000&&$siteid!=$userid){
                header('HTTP/1.1 403 Forbidden');
                header("Date:".date('r',time()));
                header('Status:403 Forbidden');
                header("error_msg:Have no right to obtain");
                die;
             }
            if(substr($userid,3)==1000&&substr($siteid,0,2)!=substr($userid,0,2)){
                header('HTTP/1.1 403 Forbidden');
                header("Date:".date('r',time()));
                header('Status:403 Forbidden');
                header("error_msg:Have no right to obtain");
